...
import cbpdbms.HeapPage;
import cbpdbms.HeapPageId;
import cbpdbms.Insert;
import cbpdbms.IntField;
import cbpdbms.Page;
import cbpdbms.SeqScan;
import cbpdbms.Transaction;
import cbpdbms.TransactionAbortedException;
import cbpdbms.Tuple;
import cbpdbms.TupleDesc;
import cbpdbms.TupleIterator;
...
public class LogTest extends SimpleDbTestBase {
	...
	void look(HeapFile hf, Transaction t, int v1, boolean present) throws DbException, TransactionAbortedException {
		...
		Page page = hf.readPage(new HeapPageId(hf.getId(), 0));
		scan.open();
		while (scan.hasNext()) {
			Tuple tu = scan.next();
	...
	void abort(Transaction t) throws DbException, TransactionAbortedException, IOException {
		...
		// step 1 write back all dirty to disk, for test
		Database.getBufferPool().flushAllPages(); // XXX defeat NO-STEAL-based
		// abort

		Debug.printLogInfo("----------------");

		// step 2 rollback acording to the log
		Database.getLogFile().logAbort(t.getId()); // does rollback too

		Database.getBufferPool().flushAllPages(); // prevent NO-STEAL-based
		// abort from
	...
	// @Test
	// public void PatchTest() throws IOException, DbException,
	...
	// "LogTest: wrong # of log records; patch failed?");
	//
	// // *** Test:
	...
	// if (same == false)
	// throw new RuntimeException(
	// "LogTest:setBeforeImage() not called? patch failed?");
	// }
	...
	public void TestAbort() throws IOException, DbException, TransactionAbortedException {
		setup();
		// step1 first insert 1， 2 and commit
		doInsert(hf1, 1, 2);
	...
	public void TestAbortCommitInterleaved() throws IOException, DbException, TransactionAbortedException {
		setup();
		doInsert(hf1, 1, 2);
		Debug.printLogInfo("**doInsert(hf1, 1, 2);");
	...
	public void TestCommitAbortCommitCrash() throws IOException, DbException, TransactionAbortedException {
		...
		setup();
		doInsert(hf1, 1, 2);
		page = hf1.readPage(new HeapPageId(hf1.getId(), 0));
		Debug.printLogInfo("**doInsert(hf1, 1, 2);");
		...
		doInsert(hf1, 5, -1);
		page = hf1.readPage(new HeapPageId(hf1.getId(), 0));
		Debug.printLogInfo("**ndoInsert(hf1, 5, -1);");
		dontInsert(hf1, 6, -1);
		page = hf1.readPage(new HeapPageId(hf1.getId(), 0));
		Debug.printLogInfo("**dontInsert(hf1, 6, -1);");
		doInsert(hf1, 7, -1);
		page = hf1.readPage(new HeapPageId(hf1.getId(), 0));
		Debug.printLogInfo("**doInsert(hf1, 7, -1);");
