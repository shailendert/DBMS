...
public class Parser {
	...
	static Predicate.Op getOp(String s) throws cbpdbms.ParsingException {
		if (s.equals("="))
		...
		throw new cbpdbms.ParsingException("Unknown predicate " + s);
	}

	static void processExpression(TransactionId tid, ZExpression wx, LogicalPlan lp)
			throws cbpdbms.ParsingException, DbException, TransactionAbortedException {
		if (wx.getOperator().equals("AND")) {
			for (int i = 0; i < wx.nbOperands(); i++) {
				if (!(wx.getOperand(i) instanceof ZExpression)) {
					throw new cbpdbms.ParsingException("Nested queries are currently unsupported.");
				}
			...
			throw new cbpdbms.ParsingException("OR expressions currently unsupported.");
		} else {
			// this is a binary expression comparing two constants
			@SuppressWarnings("unchecked")
			Vector<ZExp> ops = wx.getOperands();
			if (ops.size() != 2) {
				throw new cbpdbms.ParsingException(
			...
			if (isJoin) { // join node
				...
				if (!op2const) { // right op is a nested query
					try {
						...
						throw new cbpdbms.ParsingException("Invalid subquery " + ops.elementAt(1));
	...
	public static LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q) throws IOException, Zql.ParseException,
			cbpdbms.ParsingException, DbException, TransactionAbortedException {
		@SuppressWarnings("unchecked")
		...
		for (int i = 0; i < from.size(); i++) {
			...
			try {
				...
				e.printStackTrace();
				throw new cbpdbms.ParsingException("Table " + fromIt.getTable() + " is not in catalog");
			}
		}
		...
		if (gby != null) {
			...
			if (gbs.size() > 1) {
				throw new cbpdbms.ParsingException("At most one grouping field expression supported.");
			}
			if (gbs.size() == 1) {
				ZExp gbe = gbs.elementAt(0);
				if (!(gbe instanceof ZConstant)) {
					throw new cbpdbms.ParsingException("Complex grouping expressions (" + gbe + ") not supported.");
				}
		...
		for (int i = 0; i < selectList.size(); i++) {
			...
			if (si.getAggregate() == null && (si.isExpression() && !(si.getExpression() instanceof ZConstant))) {
				throw new cbpdbms.ParsingException("Expressions in SELECT list are not supported.");
			}
			if (si.getAggregate() != null) {
				if (aggField != null) {
					throw new cbpdbms.ParsingException("Aggregates over multiple fields not supported.");
				}
		...
		if (groupByField != null && aggFun == null) {
			throw new cbpdbms.ParsingException("GROUP BY without aggregation.");
		}
		...
		if (q.getOrderBy() != null) {
			...
			if (obys.size() > 1) {
				throw new cbpdbms.ParsingException("Multi-attribute ORDER BY is not supported.");
			}
			ZOrderBy oby = obys.elementAt(0);
			if (!(oby.getExpression() instanceof ZConstant)) {
				throw new cbpdbms.ParsingException("Complex ORDER BY's are not supported");
			}
	...
	public static DbIterator parseQuery(TransactionId tid, ZQuery q) throws IOException, Zql.ParseException,
			cbpdbms.ParsingException, DbException, TransactionAbortedException {
	...
	public static void handleQueryStatement(ZQuery s) throws TransactionAbortedException, DbException, IOException,
			cbpdbms.ParsingException, Zql.ParseException {
		// and run it
		DbIterator node;
	...
	public static void handleInsertStatement(ZInsert s) throws TransactionAbortedException, DbException, IOException,
			cbpdbms.ParsingException, Zql.ParseException {
		int id;
		try {
			...
			throw new cbpdbms.ParsingException("Unknown table : " + s.getTable());
		}
		...
		if (s.getValues() != null) {
			...
			for (ZExp e : values) {
				...
				if (!(e instanceof ZConstant))
					throw new cbpdbms.ParsingException("Complex expressions not allowed in INSERT statements.");
				...
				if (zc.getType() == ZConstant.NUMBER) {
					...
					throw new cbpdbms.ParsingException("Only string or int fields are supported.");
				}
	...
	public static void handleDeleteStatement(ZDelete s) throws TransactionAbortedException, DbException, IOException,
			cbpdbms.ParsingException, Zql.ParseException {
		int id;
		try {
			id = Database.getCatalog().getTableId(s.getTable()); // will fall
		...
		Query sdbq = new Query(new Delete(curtrans.getId(), lp.physicalPlan(curtrans.getId(), statsMap, false)),
				curtrans.getId());
		// print field names
		sdbq.start();
	...
	public static void handleTransactStatement(ZTransactStmt s) throws TransactionAbortedException, DbException,
			IOException, cbpdbms.ParsingException, Zql.ParseException {
	...
	public static LogicalPlan generateLogicalPlan(TransactionId tid, String s)
		...
		try {
			...
			throw new cbpdbms.ParsingException("Invalid SQL expression: \n \t " + e);
		} catch (IOException e) {
			throw new cbpdbms.ParsingException(e);
		}

		throw new cbpdbms.ParsingException("Cannot generate logical plan for expression : " + s);
	}
	...
	public static void processNextStatement(InputStream is) {
		try {
			...
			e.printStackTrace();
		} catch (cbpdbms.ParsingException e) {
	...
	static final String[] SQL_COMMANDS = { "select", "from", "where", "group by", "max(", "min(", "avg(", "count",
			"rollback", "commit", "insert", "delete", "values", "into" };
	...
	public static void main(String argv[]) throws IOException {
		...
		// FIXME: when use STRING as tupledesc, error
