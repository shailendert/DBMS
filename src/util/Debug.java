 ...
 * cbpdbms.Debug. For example, on the command line, use -Dcbpdbms.Debug=x, or
 * simply -Dcbpdbms.Debug to enable it at level 0. The log(level, message, ...)
...
public class Debug {
	...
	static {
		String debug = System.getProperty("cbpdbms.Debug");
		if (debug == null) {
	...
	public static void printJoinInfo(String info) {
		if (control.CommandLine.TEST_JOIN)
			System.out.println(info);
	}

	public static void printLogInfo(String info) {
		if (control.CommandLine.TEST_LOG)
			System.out.println(info);
	}
}
